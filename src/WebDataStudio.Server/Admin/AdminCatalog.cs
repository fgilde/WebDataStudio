...
{
    public static async Task<IReadOnlyList<string>> ListAsync(IDbDriver driver, IDbSession session,
        CancellationToken ct)
    {
        var sql = driver.Info.Id switch
        {
            "postgresql" => "SELECT rolname FROM pg_roles WHERE rolcanlogin ORDER BY rolname",
            "mysql" => "SELECT CONCAT(user, '@', host) FROM mysql.user ORDER BY user",
            "sqlserver" => "SELECT name FROM sys.server_principals WHERE type IN ('S','U') ORDER BY name",
        ...
        try
        {
            ...
            await using var reader = await command.ExecuteReaderAsync(ct);
            while (await reader.ReadAsync(ct)) users.Add(reader.GetString(0));
        }
        catch (DbException)
        {
            // Reading the user list is itself a privilege; without it the panel stays empty.
        }

        return users;
    }

    /// The statement is previewed before it runs, so this only renders it.
    public static string CreateStatement(IDbDriver driver, string user, string password) => driver.Info.Id switch
    {
        "postgresql" => $"CREATE ROLE {driver.Dialect.QuoteIdentifier(user)} LOGIN PASSWORD " +
                        $"{driver.Dialect.QuoteLiteral(password)};",
        "mysql" => $"CREATE USER {driver.Dialect.QuoteLiteral(user)}@'%' IDENTIFIED BY " +
                   $"{driver.Dialect.QuoteLiteral(password)};",
        "sqlserver" => $"CREATE LOGIN {driver.Dialect.QuoteIdentifier(user)} WITH PASSWORD = " +
                       $"{driver.Dialect.QuoteLiteral(password)};",
        "oracle" => $"CREATE USER {driver.Dialect.QuoteIdentifier(user)} IDENTIFIED BY " +
                    $"{driver.Dialect.QuoteIdentifier(password)};",
        _ => throw new NotSupportedException($"{driver.Info.Label} has no user management wired up"),
    };

    public static string GrantStatement(IDbDriver driver, string user, string privilege, string target) =>
        driver.Info.Id switch
        {
            "mysql" => $"GRANT {privilege} ON {target} TO {driver.Dialect.QuoteLiteral(user)}@'%';",
            _ => $"GRANT {privilege} ON {target} TO {driver.Dialect.QuoteIdentifier(user)};",
        };

    public static string RevokeStatement(IDbDriver driver, string user, string privilege, string target) =>
        driver.Info.Id switch
        {
            "mysql" => $"REVOKE {privilege} ON {target} FROM {driver.Dialect.QuoteLiteral(user)}@'%';",
            _ => $"REVOKE {privilege} ON {target} FROM {driver.Dialect.QuoteIdentifier(user)};",
        };
}
