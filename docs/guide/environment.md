...
| `WDS_DASHBOARD_FILE` | dashboards the deployment ships — a file, several files or a folder, in the studio's own shape or as Grafana JSON, told apart per file. Shown in **Tools → Dashboard** with a badge, and not editable there — see [Dashboards](dashboards.md) |
