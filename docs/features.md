...
| F14.5 | A visual builder over an OData request — fields, relations to expand, filter and order — in both the query tab and the data tab | done | odata | Wand button in the query tab and the data tab |
...
| F30.44 | A database file as a connection: uploaded from the browser or picked inside the folders the server may read, with the engine taken from the extension and a data file opened read-only through DuckDB | done | sqlite, duckdb, storage | Connection form, `WDS_FILE_ROOTS` |
| F30.45 | Connections named in the studio's own URL — a file, a download or a whole connection string — off by default, kept for the browser that opened them, each entry refused on its own with the setting that would allow it | done | all | `WDS_OPEN_FROM_URL`, `WDS_OPEN_FROM_URL_HOSTS`, `WDS_OPEN_FROM_URL_KEEP` |
| F30.46 | Where a connection somebody adds goes: the shared store as always, or the browser that made it and nowhere else | done | all | `WDS_CONNECTION_SCOPE` |
| F30.47 | Which ways in a deployment leaves open — the form and testing one, an upload, the server's own folders — each closing with its own switch and taking its button with it | done | all | `WDS_ALLOW_ADD_CONNECTION`, `WDS_ALLOW_FILE_UPLOAD`, `WDS_ALLOW_FILE_BROWSE` |
| F30.48 | A session that ends: a lifetime, a ceiling on how many connections one browser holds, a sweeper that takes the files too, and a button that ends it now | done | all | `WDS_SESSION_TTL_MINUTES`, `WDS_SESSION_MAX_CONNECTIONS`, `WDS_UPLOAD_MAX_MB` |
| F30.49 | The hosts the studio may connect to at all, checked wherever the connection came from — form, test, link, store, environment — and a target outside the list is not offered | done | all | `WDS_CONNECT_HOSTS` |
| F30.50 | The empty state of a studio somebody brings their own data to: what to bring, only the ways that are open, and what a session connection is worth knowing | done | all | Connections page |
| F31.1 | A dashboard as a canvas: twenty-four columns, drag, resize and reorder in edit mode, static in view mode, which is the default | done | all | Tools → Dashboard |
| F31.2 | Fifteen widget types — stat, gauge, bar, stacked bar, pie, treemap, line, area, stacked area, sparkline, table, list, heatmap, geo map, sankey — plus prose and a collapsible band, each asking only for the source, the column roles and the options its own form needs | done | all | Dashboard, widget settings |
| F31.3 | A widget whose rows come from several connections at once, staged and joined by the studio's own federation with a row cap per source | done | all | Dashboard, widget settings, source |
| F31.4 | One time range per dashboard, relative or absolute, reaching a statement through `$__timeFilter`, `$__from`, `$__to`, `$__fromIso`, `$__toIso` and `$__interval`, expanded per dialect | done | all | Dashboard header |
| F31.5 | Dashboard variables from a statement, a written list or a constant, single or multi, with an optional All — bound as a parameter for a single value and quoted per dialect for a list, refusing what a value may not carry | done | all | Dashboard header, variables drawer |
| F31.6 | Grafana dashboards read and written — panels, grid positions, statements, field config, variables and time range — with a sentence per thing that could not come along and the original panel kept for the way back | done | all | Dashboard, JSON view |
| F31.7 | Dashboards a deployment ships, as a file or a folder, in the studio's own shape, the older tile shape or Grafana's, told apart per file | done | all | `WDS_DASHBOARD_FILE`, `WithDashboards`, `WithGrafanaDashboards` |
| F31.8 | One validated categorical order in a light and a dark set across every theme, folding past eight rather than cycling, a legend from two series, direct labels up to four, one hue for a magnitude, reserved threshold states, and the rows of any chart one menu item away | done | all | Dashboard widgets |
| F30.51 | A deployment's own icon — a URL or a file mounted into the container, served by the studio itself — in the header, on the login screen and in the browser tab, falling back to the shipped one | done | all | `WDS_ICON`, header, login screen |
