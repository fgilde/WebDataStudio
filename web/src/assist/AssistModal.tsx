...
import {
  Alert, Badge, Button, Code, Group, Modal, ScrollArea, Stack, Switch, Text, Textarea,
} from "@mantine/core";
import { assistExplain, assistSql, type AssistReplyDto } from "../api";
...
export function AssistModal({ connectionId, sql, opened, onClose, onUseStatement }: {
  ...
  useEffect(() => {
    if (!opened) { setReply(null); setError(null); }
  }, [opened]);

  const call = async (what: "explain" | "draft") => {
    setBusy(true);
    setError(null);
    setReply(null);
    try {
